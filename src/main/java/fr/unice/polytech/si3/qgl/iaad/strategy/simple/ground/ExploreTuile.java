...
public class ExploreTuile implements Protocol {
    ...
    public ExploreTuile(Compass direction, Map contrat, Crew crew, IslandMap map) {
    ...
    public Protocol acknowledgeResults(Result result) {
        ...
        for (int i = 0; i < exploreResult.getResourcesExplored().size(); i++) {
            ...
            {
                return new ExploitResource(resource,direction,contrat,crew,map);
            }
        }
        if(containFish){
            return new SeeOcean(direction,contrat,crew,map);
        }
